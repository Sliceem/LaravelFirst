...
<div class="h-screen bg-white flex flex-col space-y-10 justify-center items-center">
    <div class="bg-white w-96 shadow-xl rounded p-5">
        ...
        <form action="{{ route('register_process') }}" method="POST" class="space-y-5 mt-5">
            ...
            <p class="text-red-500">{{ $message }}</p>
            @enderror
            <input name="password_confirmation" type="password" class="w-full h-12 border border-gray-800 rounded px-3" placeholder="Re-enter Password" />
            @error('password_confirmation')
            <p class="text-red-500">{{ $message }}</p>
            @enderror

            <div>
                <a href="{{ route('login') }}" class="font-medium text-blue-900 hover:bg-blue-300 rounded-md p-2">Got account ?</a>
            </div>
